(async function () {
  ...
  let day = qs.get("dag") || K.store.get("famoMagDay", "") || K.addDays(K.today(), 1);
  const focusId = qs.get("id");
  function dayOrders() { return S.orders.filter(o => o.day === day && o.statut !== "Facturée"); }
  function header() {
    ...
    return '<div class="page-h"><div><h1 class="h1">Magazijn</h1><p class="sub">' + K.esc(K.dateLong(day)) + ' · ' + list.length + ' bestelling' + (list.length === 1 ? "" : "en") + ' · ' + prep + ' te bereiden</p></div><span class="spacer"></span>' +
      '<div class="opt" style="flex:0 0 auto"><button type="button" data-day="' + K.today() + '"' + (day === K.today() ? ' class="on"' : "") + '>Vandaag</button><button type="button" data-day="' + K.addDays(K.today(), 1) + '"' + (day === K.addDays(K.today(), 1) ? ' class="on"' : "") + '>Morgen</button><input type="date" class="input" id="pickDay" value="' + day + '" style="width:auto;min-height:44px"></div>' +
  ...
  function verzamel(list) {
    ...
    return '<div class="card"><div class="card-h"><div><h2 class="h2">Verzamellijst</h2><p class="sub">Per product, alle bestellingen van de dag samen</p></div><span class="quiet" style="font-size:12px" id="pickCount"></span></div>' + (rows.length ? rows.map(r => { const k = r.name.toLowerCase() + "|" + r.unit; return '<div class="line' + (done[k] ? " ok" : "") + '" style="grid-template-columns:44px minmax(0,1fr) auto">' + K.c.check(!!done[k], 'data-pick="' + K.esc(k) + '" class="check big"') + '<div><b>' + K.esc(r.name) + '</b><div class="quiet" style="font-size:12px">' + r.n + ' bestelling' + (r.n === 1 ? "" : "en") + '</div></div><b class="mono" style="font-size:16px;white-space:nowrap">' + K.esc(K.qty(r.qty) + " " + K.unit(r.unit)) + '</b></div>'; }).join("") : '<div class="empty" style="margin:12px">Geen artikelen voor deze dag.</div>') + '</div>';
  }
  function perOrder(list) {
    ...
    return '<div class="card"><div class="card-h"><div><h2 class="h2">Bestellingen van de dag</h2><p class="sub">Valideer artikel per artikel, dan Klaarzetten</p></div>' + (list.some(o => o.statut === "Prête") ? '<button type="button" class="btn btn-o btn-sm" id="allDepart">Alles wat klaar is: vertrekt</button>' : "") + '</div>' + (sorted.length ? sorted.map(o => '<div class="stop" id="o-' + o.id + '"' + (o.id === focusId ? ' style="background:var(--p-soft)"' : "") + '>' + K.c.avatar(o.client) + '<div style="flex:1;min-width:0"><div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap"><b>' + K.esc(o.client) + '</b><span class="quiet mono" style="font-size:11px">' + K.esc(o.ref) + '</span>' + (o.notes ? '<span class="tag" title="' + K.esc(o.notes) + '">nota</span>' : "") + '</div><div class="quiet" style="font-size:12px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">' + K.esc(S.lineTxt(o)) + '</div></div>' + K.stCell(o.statut) + '<div style="display:flex;gap:6px"><a class="btn btn-o btn-sm" href="/order.html?id=' + encodeURIComponent(o.id) + '">Open</a>' + S.nextAction(o) + '</div></div>').join("") : '<div class="empty" style="margin:12px">Nog geen bestellingen voor ' + K.esc(K.relDay(day).toLowerCase()) + '. Klanten kunnen bestellen tot 22:00.</div>') + '</div>';
  }
  function bord() {
    const list = S.orders.filter(o => o.statut !== "Facturée");
    const cols = [["Reçue", "Te bereiden", "var(--st-new)"], ["Prête", "Klaar", "var(--st-ready)"], ["Sortie en livraison", "Onderweg", "var(--st-road)"]];
  ...
  function render() {
    ...
    K.$$("[data-day]", page).forEach(b => { b.onclick = () => { day = b.dataset.day; K.store.set("famoMagDay", day); render(); }; });
    page.querySelector("#pickDay").onchange = e => { if (e.target.value) { day = e.target.value; K.store.set("famoMagDay", day); render(); } };
    page.querySelector("#print").onclick = () => S.openPicking(list, K.dateLong(day));
    const ad = page.querySelector("#allDepart"); if (ad) ad.onclick = async () => { const ready = list.filter(o => o.statut === "Prête"); if (!(await K.confirm({ title: ready.length + " bestellingen vertrekken?", text: "Ze gaan allemaal op Onderweg. Artikelen kunnen daarna niet meer gewijzigd worden.", yes: "Vertrekken" }))) return; for (const o of ready) { try { await K.api("/api/updateorder", { json: { id: o.id, statut: "Sortie en livraison", skipStock: true } }); } catch (err) { K.toast(o.client + ": " + err.message, { kind: "err" }); } } await S.load(true); render(); K.toast("Ronde onderweg"); };
    ...
    K.on(page, "click", "[data-pick]", (e, t) => { const done = K.store.get("famoPick:" + day, {}); const k = t.dataset.pick; done[k] = !done[k]; K.store.set("famoPick:" + day, done); t.classList.toggle("on", !!done[k]); t.closest(".line").classList.toggle("ok", !!done[k]); cnt(); });
