(async function () {
  ...
  function clientPanel(c, prefill) {
    ...
    let gen = false; const g = p.el.querySelector("#cGen"); if (g) g.onclick = () => { gen = !gen; K.setOn(g, gen); };
  ...
  function productPanel(p) {
    ...
    const pn = K.panel({ title: p ? p.nom : "Nieuw product", sub: p ? "Product bewerken" : "Verschijnt in de klantcatalogus zodra actief", body:
      ...
      K.c.field("Categorie", K.c.input("pCat", { value: v.cat, placeholder: "bv. Vis, Schelpdieren, Schaaldieren", attrs: ' list="cats"' }) + '<datalist id="cats">' + Array.from(new Set((D.products || []).map(x => x.cat).filter(Boolean))).map(x => '<option value="' + K.esc(x) + '">').join("") + '</datalist>', {}) +
  ...
  function producten() {
    const all = D.products || []; const groups = {}; all.forEach(p => { const g = K.cat(p.cat); (groups[g] = groups[g] || []).push(p); });
