(async function () {
  ...
  function aanvragen() {
    ...
    const contact = a => '<div class="muted" style="font-size:12.5px">' + K.esc(a.contactpersoon) + ' · <span class="tag">' + (a.taal === "FR" ? "Français" : "Nederlands") + '</span> · ' + (a.telefoon ? '<a href="tel:' + K.esc(String(a.telefoon).replace(/\s+/g, "")) + '">' + K.esc(a.telefoon) + '</a>' : '<span class="quiet">geen telefoon</span>') + ' · <a href="mailto:' + K.esc(a.email) + '">' + K.esc(a.email) + '</a></div>';
  ...
  function clientPanel(c, prefill) {
    ...
    p.el.querySelector("#cOk").onclick = async () => {
      ...
      try {
        const d = await post({ action: "saveClient", id: c ? c.id : undefined, nom, adresse: val("cAdr"), tel: val("cTel"), email: val("cMail"), btw: val("cBtw"), klantnr: val("cNr"), taal, user: c ? val("cUser") : "", password: pw || undefined, generate: pw ? false : (c ? gen : true), sendMail });
  ...
  function productOrder() {
    ...
    page.innerHTML = head("Volgorde van de catalogus", '<button type="button" class="btn btn-ghost btn-sm" id="orderCancel">Annuleren</button><button type="button" class="btn btn-p btn-sm" id="orderSave">Volgorde bewaren</button>') +
      '<div class="content" style="padding-top:16px"><p class="sub" style="white-space:normal;margin:0 0 4px">Sleep aan <span style="display:inline-flex;vertical-align:middle">' + K.icon("grip") + '</span> of gebruik ▲▼ om categorieën en producten te verplaatsen. Dit is ook de volgorde in de klantcatalogus en bij Invoeren.</p>' +
      '<div data-cat-list style="display:flex;flex-direction:column;gap:12px">' + cats.map(g => '<div class="grp ccat" data-cat="' + K.esc(g) + '"><div class="grp-h" style="gap:6px"><span class="grip cgrip" title="Categorie verslepen">' + K.icon("grip") + '</span>' + K.esc(g) + ' <small>' + groups[g].length + '</small>' + '<span class="mvbtns"><button type="button" class="ibtn" data-mvc="-1" aria-label="' + K.esc("Categorie " + g) + ' omhoog">▲</button><button type="button" class="ibtn" data-mvc="1" aria-label="' + K.esc("Categorie " + g) + ' omlaag">▼</button></span>' + '</div><div data-plist>' +
