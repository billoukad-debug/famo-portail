(function () {
  ...
  async function renderBestellingen() {
    ...
    shell("bestellingen", '<div class="mlist">' + (list.length ? list.map(card).join("") : K.c.empty(ordFilter === "lopend" ? "Geen lopende bestellingen" : "Niets in deze lijst", ordFilter === "lopend" ? "Bestel vóór 22:00 voor levering morgen." : "", ordFilter === "lopend" ? '<a class="btn btn-p btn-sm" href="#/catalogus" style="margin-top:6px">Naar de catalogus</a>' : "")) + '</div>', topbar("Mijn bestellingen", cat.client.nom) .replace("</div></div>", "</div>" + chips + "</div>"));
