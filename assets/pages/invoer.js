(async function () {
  ...
  function render() {
    ...
    const list = products.filter(p => !q || (p.nom + " " + K.cat(p.cat)).toLowerCase().includes(q));
    page.innerHTML = '<div class="page-h"><div><h1 class="h1">Bestelling invoeren</h1><p class="sub">Voor een klant die belt, mailt of appt</p></div><span class="spacer"></span><div class="opt" style="flex:0 0 auto">' + ["Telefoon", "WhatsApp", "E-mail", "Toonbank"].map(b => '<button type="button" data-bron="' + b + '"' + (b === bron ? ' class="on"' : "") + '>' + b + '</button>').join("") + '</div></div>' +
      ...
      (clientId ? (list.length ? list.map(p => '<div class="line" style="grid-template-columns:minmax(0,1fr) auto auto"><div><b>' + K.esc(p.nom) + '</b><div class="quiet" style="font-size:12px">' + K.esc(K.unit(p.unite)) + " · " + K.esc(K.cat(p.cat)) + '</div></div><div class="pp"><b class="mono">' + K.eur(p.prix) + '</b>' + (p.prix < p.base ? '<s class="mono">' + K.eur(p.base) + '</s>' : "") + '</div>' + K.c.stepper(p.id, items[p.id] || 0, { step: isKg(p) ? 0.5 : 1 }) + '</div>').join("") : '<div class="empty" style="margin:12px">Geen product gevonden.</div>') : '<div class="state"><b>Kies eerst een klant</b><p class="sub">De prijzen in de lijst zijn de afgesproken prijzen van die klant.</p></div>') + '</div>' +
