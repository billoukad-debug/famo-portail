(function () {
  ...
  app.innerHTML = '<div class="start">' +
    '<header class="start-hd"><a class="brand" href="/"><span class="logo" style="width:36px;height:36px">F</span><span><b style="font-size:16px">FAMO Seafood</b><small>' + K.t("Verse vis en zeevruchten · Antwerpen") + '</small></span></a>' +
    '<nav class="mini">' + K.langSwitch() + '<a href="/aanvraag.html">' + K.t("Toegang aanvragen") + '</a></nav></header>' +
    '<main class="start-body"><section class="hero"><h1>' + K.t("Verse vis bestellen,<br>zo simpel als een berichtje.") + '</h1><p>' + K.t("Bestel vandaag vóór 22:00 en wij leveren morgen in Antwerpen en omstreken. U ziet uw afgesproken prijzen, kiest zelf de leverdag en vindt uw leveringsbonnen en facturen terug in het portaal.") + '</p>' +
    ...
    (denied ? K.c.ok(K.t("U bent afgemeld.")) : "") +
    ...
    K.c.field(K.t("Wachtwoord"), '<div style="position:relative">' + K.c.input("pw", { type: "password", attrs: ' autocomplete="current-password" required style="padding-right:76px"' }) + '<button type="button" class="btn btn-ghost btn-sm" id="togglePw" style="position:absolute;right:4px;top:2px;min-height:40px">' + K.t("Tonen") + '</button></div>', { id: "fPw", for: "pw" }) +
    '<div id="loginErr"></div>' +
    '<button type="submit" class="btn btn-p" id="loginBtn" style="min-height:50px;font-size:15px">' + K.t("Aanmelden") + '</button>' +
    '<div style="display:flex;justify-content:space-between;font-size:13px;flex-wrap:wrap;gap:8px"><a href="/wachtwoord.html">' + K.t("Wachtwoord vergeten?") + '</a><a href="/aanvraag.html">' + K.t("Nog geen klant? Toegang aanvragen") + '</a></div></form></main>' +
  ...
  document.getElementById("proof").innerHTML = '<div><b>22:00</b>' + K.t("besteldeadline") + '</div><div><b>' + K.t("ma–za") + '</b>' + K.t("levering, niet op zondag") + '</div><div><b>' + K.t("Gratis") + '</b>' + K.t("levering") + '</div>';
