...
(function () {
  ...
  function renderChrome() {
    ...
    who.innerHTML = on ? '<span class="pill ' + (S.role === "admin" ? "pill-accent" : "pill-info") + '">' + (S.role === "admin" ? "Beheerder" : "Team") + "</span> " + '<a href="/" target="_blank" style="color:#fff;margin-left:8px">Klantportaal</a>' + (S.role === "admin" ? '<a href="/beheer" style="color:#fff;margin-left:8px">Beheer</a>' : "") + ' <button class="btn btn-sm btn-ghost" id="logout" style="color:#fff">Afmelden</button>' : "";
  ...
  function renderMeer() {
    const s = K.sheet({ title: "Meer", body: '<div class="big-actions">' + (S.role === "admin" ? '<a class="btn btn-outline" href="/beheer">' + K.icon("settings") + " Beheer</a>" : "") + '<a class="btn btn-outline" target="_blank" href="/doc/picklijst?dag=' + K.todayISO() + '">' + K.icon("print") + ' Picklijst vandaag</a><a class="btn btn-outline" target="_blank" href="/doc/picklijst?dag=' + K.addDays(K.todayISO(), 1) + '">' + K.icon("print") + ' Picklijst morgen</a><a class="btn btn-outline" target="_blank" href="/doc/leveringsbonnen?dag=' + K.todayISO() + '">' + K.icon("print") + ' Alle leveringsbonnen vandaag</a><a class="btn btn-outline" href="/" target="_blank">Klantportaal bekijken</a><button class="btn btn-ghost" id="lo">Afmelden</button></div>', footer: false });
  ...
  function renderBoard() {
    ...
    main.innerHTML = '<div class="section-head"><div><h1>' + esc(K.dateNl(today, true)) + '</h1><div class="muted small">' + K.plural(S.data.orders.filter((o) => o.status !== "geleverd").length, "open bestelling", "open bestellingen") + '</div></div><div class="row wrap"><a class="btn btn-outline" target="_blank" href="/doc/picklijst?dag=' + today + '">' + K.icon("print") + ' Picklijst</a><a class="btn btn-outline" target="_blank" href="/doc/leveringsbonnen?dag=' + today + '">' + K.icon("print") + ' Leveringsbonnen</a>' + (S.data.orders.some((o) => o.status === "klaar" && o.deliveryDate <= today) ? '<button class="btn btn-accent" id="shipAll">' + K.icon("truck") + ' Ronde vertrekt</button>' : "") + '<button class="iconbtn" id="refresh" aria-label="Vernieuwen">' + K.icon("refresh") + "</button></div></div>" +
    ...
    if ($("#shipAll")) $("#shipAll").onclick = async () => {
      const n = S.data.orders.filter((o) => o.status === "klaar" && o.deliveryDate <= today).length;
      if (!(await K.confirm({ title: "Ronde vertrekt?", text: K.plural(n, "klaarstaande bestelling", "klaarstaande bestellingen") + " voor vandaag gaan op Onderweg. Elke klant met e-mailadres krijgt een bericht.", yes: "Ja, vertrekken" }))) return;
      await K.busy($("#shipAll"), async () => { try { const r = await K.api("team/onderweg-alles", { body: { dag: today } }); K.toast(K.plural(r.shipped, "bestelling", "bestellingen") + " onderweg" + (r.mails ? " · " + K.plural(r.mails, "klant", "klanten") + " verwittigd" : "") + (r.failed.length ? " · " + r.failed.length + " mislukt" : ""), r.failed.length ? "bad" : "ok", 5000); await load(); renderBoard(); } catch (err) { K.toast(err.message, "bad"); } });
    };
  ...
  async function openOrder(id) {
    ...
    function drawOrder() {
      ...
      const head = '<div class="row spread wrap" style="margin-bottom:6px"><div><div class="muted small">' + esc(o.ref) + " · besteld " + esc(K.dateShort(o.date)) + (o.source !== "Klantportaal" ? " · " + esc(o.source) : "") + '</div><div><b>Levering ' + esc(K.dateNl(o.deliveryDate, true)) + "</b></div></div>" + K.chip(o.status, o.statusLabel) + "</div>" + K.timeline(idx) +
        '<div class="card flat" style="padding:10px 12px;margin:10px 0"><div class="row spread wrap"><div><b>' + esc(c.name || "—") + '</b> <span class="muted small">' + esc(c.number || "") + '</span><div class="small">' + esc(c.address || "").replace(/\n/g, ", ") + "</div></div>" + '<div class="row wrap">' + (c.address ? '<a class="btn btn-sm btn-outline" target="_blank" rel="noopener" href="' + esc(K.mapsUrl(c.address)) + '">' + K.icon("truck") + " Route</a>" : "") + (c.phone ? '<a class="btn btn-sm btn-outline" href="tel:' + esc(String(c.phone).replace(/\s/g, "")) + '">' + K.icon("phone") + " Bellen</a>" : "") + "</div></div>" + (c.notes ? '<div class="small muted" style="margin-top:6px">ℹ️ ' + esc(c.notes) + "</div>" : "") + "</div>" +
      ...
      if (o.status === "ontvangen") {
        ...
        const editable = o.status !== "geleverd";
        lines = '<div class="card pad-0 flat"><table class="table"><tbody>' + o.lines.map((l, i) => '<tr><td class="num strong" style="width:110px">' + K.qty(l.qty) + " " + esc(K.unit(l.unit, l.qty)) + "</td><td>" + esc(l.name) + (l.comment ? '<div class="small" style="color:var(--accent-2)">' + esc(l.comment) + "</div>" : "") + '</td><td class="num muted">' + (l.priceCents != null ? K.eur(Math.round(l.priceCents * l.qty)) : "") + "</td>" + (editable ? '<td style="width:1%"><button class="btn btn-sm btn-ghost" data-edit="' + i + '">Wijzig</button></td>' : "") + "</tr>").join("") + '</tbody></table><div class="row spread" style="padding:10px 12px;border-top:1px solid var(--line-2)"><span class="muted">Totaal excl. btw</span><b class="num">' + K.eur(o.totalCents) + "</b></div></div>" + (editable ? '<p class="small muted" style="margin-top:6px">Artikel geweigerd of beschadigd aan de deur? Pas de lijn aan vóór u de levering afrondt; de factuur volgt de werkelijk geleverde hoeveelheden.</p>' : "");
      }
      let after = "";
    ...
    function deliverSheet() {
      ...
      $("#ok", s.el).onclick = async () => {
        ...
        const r = await act("geleverd", { ontvanger: name, handtekening: pad.isEmpty() ? null : pad.dataUrl(), betaald: $("#cash", b).checked }, $("#ok", s.el), "Levering afgerond");
  ...
  async function renderNieuw() {
    ...
    if (!N.client) {
      main.innerHTML = '<h1 style="margin-bottom:10px">Nieuwe bestelling</h1><p class="muted">Voor een klant die belt of een bericht stuurt. Kies de klant' + (S.role === "admin" ? ' of <a href="/beheer">maak een nieuwe klant aan</a>' : "") + ':</p><div class="searchbox" style="margin-bottom:10px">' + K.icon("search") + '<input class="input" id="cq" placeholder="Zoek klant…" autofocus></div><div class="card pad-0 flat"><div class="list" id="cl"></div></div>';
