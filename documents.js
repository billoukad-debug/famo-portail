window.FamoDocuments=(()=>{
  ...
  let COMPANY={
    nom:"Famo Trading BV",
    adresse:"Jezusstraat 34",
    cp:"2000 Antwerpen, België",
    tva:"BE 0788.705.713",
    tel:"",
    iban:"",
    bic:""
  ...
  function setCompany(cfg){
    cfg=cfg||{};
    COMPANY={
      nom:cfg.nom||cfg.bedrijfsnaam||COMPANY.nom,
      adresse:cfg.adresse||cfg.adres||COMPANY.adresse,
      cp:cfg.cp||cfg.plaats||COMPANY.cp,
      tva:cfg.tva||cfg.btw||COMPANY.tva,
      tel:cfg.tel||cfg.telefoon||COMPANY.tel||"",
      iban:(cfg.iban||COMPANY.iban||"").trim(),
      bic:(cfg.bic||COMPANY.bic||"").trim()
    };
    return COMPANY;
  }
  ...
  function build(order,type){
    ...
    const bank=COMPANY.iban
      ? ('<div class="bank"><b>Bankgegevens</b><br>'+esc(COMPANY.nom)+'<br>IBAN: '+esc(COMPANY.iban)+(COMPANY.bic?'<br>BIC: '+esc(COMPANY.bic):'')+'</div>')
      : '<div class="bank"><b>Betaling</b><br>Bankgegevens nog niet ingevuld in Configuratie (Aan de slag).</div>';
    const foot=credit
  ...
  return{build,number,filename,parse,eur,esc,date,setCompany,getCompany:()=>COMPANY};
})();
