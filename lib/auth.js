...
const TTL_DOC_MS = 3 * 365 * 24 * 3600 * 1000;
...
// ---- Klantsessie ---------------------------------------------------------------
const TTL_CLIENT_SHORT_MS = 12 * 3600 * 1000;
function setClientSession(res, clientId, passwordVersion, opts) {
  const ttl = opts && opts.short ? TTL_CLIENT_SHORT_MS : TTL_CLIENT_MS;
  const exp = Date.now() + ttl;
  appendSetCookie(res, cookieString(COOKIE_CLIENT, sign({ k: "klant", id: clientId, pv: passwordVersion || "", short: !!(opts && opts.short), exp }), Math.floor(ttl / 1000)));
}
...
function clientSession(req) {
  ...
  return p && p.k === "klant" && p.id ? { clientId: p.id, passwordVersion: p.pv || "", short: !!p.short, exp: p.exp } : null;
}
...
/** Glijdende sessies: wie actief is, wordt niet midden in de shift afgemeld. */
function renewSessions(req, res) {
  const t = teamSession(req);
  if (t && t.exp - Date.now() < TTL_TEAM_MS / 2) setTeamSession(res, t.role);
  const c = clientSession(req);
  if (c && c.exp - Date.now() < (c.short ? TTL_CLIENT_SHORT_MS : TTL_CLIENT_MS) / 2) setClientSession(res, c.clientId, c.passwordVersion, { short: c.short });
}
...
function rateLimited(key, max, windowMs) {
  const now = Date.now();
  if (_buckets.size > 5000) { let n = 0; for (const key of _buckets.keys()) { _buckets.delete(key); if (++n >= 1000) break; } }
...
module.exports = {
  ...
  setTeamSession, clearTeamSession, teamSession, isAdmin, renewSessions,
