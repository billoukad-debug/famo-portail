...
// ---- Klantsessie ---------------------------------------------------------------
function setClientSession(res, clientId, passwordVersion) {
  const exp = Date.now() + TTL_CLIENT_MS;
  appendSetCookie(res, cookieString(COOKIE_CLIENT, sign({ k: "klant", id: clientId, pv: passwordVersion || "", exp }), Math.floor(TTL_CLIENT_MS / 1000)));
}
...
function clientSession(req) {
  ...
  return p && p.k === "klant" && p.id ? { clientId: p.id, passwordVersion: p.pv || "", exp: p.exp } : null;
}
...
function renewSessions(req, res) {
  ...
  if (c && c.exp - Date.now() < TTL_CLIENT_MS / 2) setClientSession(res, c.clientId, c.passwordVersion);
}
