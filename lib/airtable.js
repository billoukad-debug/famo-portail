...
// - maximaal 4 gelijktijdige verzoeken (Airtable laat 5/s per base toe)
// - 429: wachten volgens Retry-After (of oplopend) en opnieuw proberen
// - netwerkfout/5xx: enkel GET en DELETE worden herhaald (POST/PATCH nooit,
//   anders kan een al aangemaakte bestelling dubbel ontstaan)
...
const RETRY_429_MS = [2000, 5000, 10000];
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

// ---- Eenvoudige semafoor: nooit meer dan MAX_CONCURRENT verzoeken tegelijk ------
let inFlight = 0;
const waiting = [];
...
function isIdempotent(method) { return method === "GET" || method === "DELETE"; }
...
async function call(url, options, attempt = 1) {
  ...
  try {
    ...
    release();
    if (isIdempotent(method) && attempt < MAX_ATTEMPTS) { await sleep(400 * attempt); return call(url, options, attempt + 1); }
    throw new AirtableError(0, "Airtable is niet bereikbaar: " + String(e && e.message || e));
  }
  let text = "";
  try { text = await res.text(); } finally { release(); }
  if (res.status === 429 && attempt < MAX_ATTEMPTS) {
    const ra = Number(res.headers && res.headers.get && res.headers.get("retry-after"));
    const wait = Number.isFinite(ra) && ra > 0 ? Math.min(ra * 1000, 12000) : RETRY_429_MS[attempt - 1] || 10000;
    await sleep(wait);
    return call(url, options, attempt + 1);
  }
  if (res.status >= 500 && isIdempotent(method) && attempt < MAX_ATTEMPTS) {
