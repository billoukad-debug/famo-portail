...
function pathSegments(req) {
  // Vercel: catch-all `api/[...path].js` levert req.query.path (array of string).
  const q = req.query || {};
  let p = q.path;
...
async function guard(fn, req, res) {
  try {
    ...
    const known = e && (e.name === "HttpError" || e.name === "DomainError" || e.name === "AirtableError");
    const message = known ? e.message : "Er liep iets mis. Probeer het opnieuw of bel ons.";
    if (!known || status >= 500) console.error("[api]", (e && e.stack) || e);
    if (res.headersSent) return;
    json(res, status, Object.assign({ error: message }, (e && e.extra) || {}));
  }
}
