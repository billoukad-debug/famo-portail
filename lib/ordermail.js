...
// Ratio calculé : #5F574E sur #fff (carte du mail) = 7,1:1 — l'e-mail garde
// l'hex opaque volontairement (rgba est mal géré par les clients mail).
const MUTED = "color:#5F574E;font-size:13px;line-height:1.5;" + FONT;
// Micro-libellés en capitales suivies (jetons : encre .62 ≈ #5F574E sur crème).
const LABEL = "font-size:11px;letter-spacing:0.5px;text-transform:uppercase;color:#5F574E;" + FONT;
...
function brandRow(name) {
  ...
  return '<tr><td style="padding:20px 24px 0 24px;">' +
    ...
    '<td width="30" height="30" style="width:30px;height:30px;background:#0C6157;border-radius:8px;text-align:center;vertical-align:middle;">' +
      '<span style="font-family:Georgia,\'Times New Roman\',serif;font-size:17px;line-height:30px;color:#ffffff;">F</span></td>' +
    '<td style="padding-left:11px;font-size:11.5px;font-weight:bold;letter-spacing:2px;color:#191512;' + FONT + '">' + brand + "</td>" +
    "</tr></table></td></tr>";
}
...
function shell(title, inner, brandName) {
  return '<!doctype html><html lang="nl"><head><meta charset="utf-8">' +
    ...
    '<table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="background:#FAF6F0;">' +
    '<tr><td align="center" style="padding:28px 12px;">' +
    '<table role="presentation" cellpadding="0" cellspacing="0" border="0" width="600" style="width:600px;max-width:100%;background:#ffffff;border:1px solid #E5DDD3;border-radius:12px;">' +
    brandRow(brandName) +
    inner +
...
function headerRow(title, subtitle) {
  return '<tr><td style="padding:18px 24px 6px 24px;">' +
    '<div style="font-family:Georgia,\'Times New Roman\',serif;font-size:23px;font-weight:normal;letter-spacing:-0.2px;color:#191512;">' + esc(title) + "</div>" +
...
function factsRow(pairs) {
  const rows = pairs.filter(p => p && p[1]).map(p =>
    ...
    '<td style="padding:4px 0;vertical-align:top;font-size:13.5px;color:#191512;' + FONT + '">' + esc(p[1]) + "</td></tr>"
  ).join("");
  if (!rows) return "";
...
function linesRow(rows, total) {
  const TH = 'style="padding:8px 10px;background:#F2ECE3;border-bottom:1px solid #E5DDD3;font-size:10.5px;letter-spacing:0.5px;text-transform:uppercase;font-weight:normal;color:#5F574E;' + FONT + '"';
  const head = "<tr>" +
  ...
  const body = rows.map(r => {
    ...
    return "<tr>" +
      '<td style="padding:10px;border-bottom:1px solid #E5DDD3;font-size:13.5px;color:#191512;' + FONT + '">' +
        esc(r.name) + (r.comment ? '<div style="margin-top:2px;' + MUTED + 'font-size:12px;">' + esc(r.comment) + "</div>" : "") + "</td>" +
      '<td align="right" style="padding:10px;border-bottom:1px solid #E5DDD3;font-size:13.5px;color:#191512;white-space:nowrap;' + FONT + '">' +
        esc(r.qty) + " " + esc(nlUnit(r.unit)) + "</td>" +
      '<td align="right" style="padding:10px;border-bottom:1px solid #E5DDD3;font-size:13.5px;color:#191512;white-space:nowrap;' + FONT + '">' +
        (sub == null ? "—" : esc(eur(sub))) + "</td></tr>";
  }).join("");
  const foot = '<tr><td colspan="2" align="right" style="padding:12px 10px 4px 10px;border-top:2px solid #191512;font-size:15px;font-weight:bold;color:#191512;' + FONT + '">Totaal excl. btw</td>' +
    '<td align="right" style="padding:12px 10px 4px 10px;border-top:2px solid #191512;font-size:16px;font-weight:bold;color:#191512;white-space:nowrap;' + FONT + '">' + esc(eur(total)) + "</td></tr>";
...
function noteRow(label, value) {
  ...
  return '<tr><td style="padding:14px 24px 0 24px;">' +
    '<table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="background:#FAF6F0;border:1px solid #E5DDD3;border-radius:12px;">' +
    '<tr><td style="padding:11px 14px;font-size:13px;line-height:1.55;color:#8A6110;' + FONT + '">' +
...
function buttonRow(label, href) {
  ...
  return '<tr><td style="padding:18px 24px 20px 24px;">' +
    '<a href="' + esc(href) + '" style="display:inline-block;padding:13px 22px;background:#0C6157;color:#ffffff;text-decoration:none;border-radius:8px;font-size:14px;line-height:18px;font-weight:bold;' + FONT + '">' +
    esc(label) + "</a></td></tr>";
}

function footerRow(text) {
  return '<tr><td style="padding:16px 24px 20px 24px;border-top:1px solid #E5DDD3;font-size:12px;line-height:1.6;color:#5F574E;' + FONT + '">' +
    esc(text).replace(/\n/g, "<br>") + "</td></tr>";
}
...
function paragraphRow(text) {
  if (!text) return "";
  return '<tr><td style="padding:14px 24px 4px 24px;font-size:14px;line-height:1.55;color:#191512;' + FONT + '">' +
    esc(text).replace(/\n/g, "<br>") + "</td></tr>";
}
...
function credentialsRow(pairs) {
  const rows = pairs.filter(p => p && p[1]).map(p =>
    ...
    '<td style="padding:5px 0;vertical-align:top;font-family:Consolas,Menlo,monospace;font-size:14px;color:#191512;">' + esc(p[1]) + "</td></tr>"
  ).join("");
  if (!rows) return "";
  return '<tr><td style="padding:14px 24px 0 24px;">' +
    '<table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="background:#FAF6F0;border:1px solid #E5DDD3;border-radius:12px;">' +
