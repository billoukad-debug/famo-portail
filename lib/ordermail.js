...
function shell(title, inner) {
  return '<!doctype html><html lang="nl"><head><meta charset="utf-8">' +
    ...
    '<table role="presentation" cellpadding="0" cellspacing="0" border="0" width="600" style="width:600px;max-width:100%;background:#ffffff;border:1px solid #E5DDD3;border-radius:10px;">' +
    inner +
...
function headerRow(title, subtitle) {
  return '<tr><td style="padding:22px 24px 6px 24px;">' +
    '<div style="font-size:19px;font-weight:bold;color:#191512;' + FONT + '">' + esc(title) + "</div>" +
...
function factsRow(pairs) {
  const rows = pairs.filter(p => p && p[1]).map(p =>
    ...
    '<td style="padding:3px 0;vertical-align:top;font-size:13.5px;color:#191512;' + FONT + '">' + esc(p[1]) + "</td></tr>"
  ).join("");
  if (!rows) return "";
...
function linesRow(rows, total) {
  const head = '<tr>' +
    '<th align="left" style="padding:7px 8px;background:#F2ECE3;border-bottom:1px solid #E5DDD3;font-size:11px;text-transform:uppercase;color:#5F574E;' + FONT + '">Artikel</th>' +
    '<th align="right" style="padding:7px 8px;background:#F2ECE3;border-bottom:1px solid #E5DDD3;font-size:11px;text-transform:uppercase;color:#5F574E;' + FONT + '">Aantal</th>' +
    '<th align="right" style="padding:7px 8px;background:#F2ECE3;border-bottom:1px solid #E5DDD3;font-size:11px;text-transform:uppercase;color:#5F574E;' + FONT + '">Subtotaal</th></tr>';
  const body = rows.map(r => {
    const qty = Number(String(r.qty).replace(",", ".")) || 0;
    const sub = r.price == null ? null : r.price * qty;
    return "<tr>" +
      '<td style="padding:9px 8px;border-bottom:1px solid #EFE7DB;font-size:13.5px;color:#191512;' + FONT + '">' +
        esc(r.name) + (r.comment ? '<div style="' + MUTED + 'font-size:12px;">' + esc(r.comment) + "</div>" : "") + "</td>" +
      '<td align="right" style="padding:9px 8px;border-bottom:1px solid #EFE7DB;font-size:13.5px;color:#191512;white-space:nowrap;' + FONT + '">' +
        esc(r.qty) + " " + esc(nlUnit(r.unit)) + "</td>" +
      '<td align="right" style="padding:9px 8px;border-bottom:1px solid #EFE7DB;font-size:13.5px;color:#191512;white-space:nowrap;' + FONT + '">' +
        (sub == null ? "—" : esc(eur(sub))) + "</td></tr>";
  }).join("");
  const foot = '<tr><td colspan="2" align="right" style="padding:11px 8px;font-size:14px;font-weight:bold;color:#191512;' + FONT + '">Totaal</td>' +
    '<td align="right" style="padding:11px 8px;font-size:14px;font-weight:bold;color:#191512;white-space:nowrap;' + FONT + '">' + esc(eur(total)) + "</td></tr>";
...
function buttonRow(label, href) {
  ...
  return '<tr><td style="padding:16px 24px 4px 24px;">' +
    '<a href="' + esc(href) + '" style="display:inline-block;padding:11px 18px;background:#0C6157;color:#ffffff;text-decoration:none;border-radius:8px;font-size:13.5px;font-weight:bold;' + FONT + '">' +
    esc(label) + "</a></td></tr>";
}

function footerRow(text) {
  return '<tr><td style="padding:18px 24px 22px 24px;border-top:1px solid #EFE7DB;' + MUTED + '">' +
    esc(text).replace(/\n/g, "<br>") + "</td></tr>";
}
