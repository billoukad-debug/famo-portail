...
function linesTable(lines, totals, opts = {}) {
  ...
  const body = lines.map((l) => { const off = dom.isUnavailable(l); const st = off ? "text-decoration:line-through;color:" + C.muted + ";" : ""; return `<tr><td ${td}><span style="${st}">${esc(l.name)}</span>${off ? ` <span style="font-size:11px;font-weight:700;color:#8e1f0b;">NIET GELEVERD</span>` : ""}${l.comment && !off ? `<div style="${MUTED}font-size:12px;">${esc(l.comment)}</div>` : ""}</td>` +
    `<td ${tdn}><span style="${st}">${off ? "—" : esc(dom.fmtQty(l.qty)) + " " + esc(dom.unitPlural(l.unit, l.qty))}</span></td>` +
    (withPrices ? `<td ${tdn}>${Number.isFinite(l.priceCents) && !off ? esc(dom.fmtEur(l.priceCents)) : "—"}</td><td ${tdn}>${Number.isFinite(l.priceCents) && !off ? esc(dom.fmtEur(dom.lineTotalCents(l))) : "—"}</td>` : "") + "</tr>"; }).join("");
...
function orderTeam(ctx) {
  ...
  const subject = `Bestelling voor ${dom.fmtDateNl(order.deliveryDate, { short: true, noYear: true })} · ${client.name || "onbekende klant"} · ${dom.fmtEur(order.totalCents)} · ${order.ref}`;
  const html = shell({
...
function orderClient(ctx) {
  const { order, client, company } = ctx;
  const subject = `Uw bestelling voor ${dom.fmtDateNl(order.deliveryDate, { noYear: true })} is ontvangen (${order.ref})`;
  const html = shell({
    ...
    body: heading("Bedankt voor uw bestelling", `Wij leveren op ${dom.fmtDateNl(order.deliveryDate)}.`) +
      ...
      para(`Zolang uw bestelling op Ontvangen staat, kunt u ze in het portaal annuleren en opnieuw plaatsen. Daarna: ${company.phone ? "bel ons op " + company.phone : "neem contact met ons op"}.`) +
...
function orderDelivered(ctx) {
  ...
  const pay = order.paid ? "Deze factuur werd contant betaald bij levering. U hoeft niets meer over te schrijven." : (company.iban ? `Gelieve ${dom.fmtEur(vat.inclCents)} over te schrijven op ${company.iban}${company.bic ? " (BIC " + company.bic + ")" : ""} vóór ${dom.fmtDateShort(ctx.dueDate || dom.todayISO())}, met vermelding van ${order.invoiceNumber}.` : "");
  const html = shell({
    ...
    body: heading("Uw levering is afgerond", order.receivedBy ? `In ontvangst genomen door ${order.receivedBy} op ${dom.fmtDateTimeNl(order.deliveredAt)}.` : `Geleverd op ${dom.fmtDateTimeNl(order.deliveredAt)}.`) +
      facts([["Factuurnummer", order.invoiceNumber], ["Factuurdatum", dom.fmtDateShort(dom.todayISO(new Date(order.invoicedAt || Date.now())))], ["Bestelreferentie", order.ref], ["Betaling", order.paid ? "Contant betaald bij levering" : company.paymentTerms + (ctx.dueDate ? " · te betalen vóór " + dom.fmtDateShort(ctx.dueDate) : "")]]) +
...
function credentials(ctx) {
  ...
  const html = shell({
    ...
    body: heading("Welkom bij het bestelportaal", `Beste ${ctx.contact ? ctx.contact + " (" + client.name + ")" : client.name},`) +
