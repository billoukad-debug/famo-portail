...
function orderClient(ctx) {
  ...
  return { to: client.email, subject, html, text, replyTo: company.email || company.opsEmail, idempotencyKey: `kade:${order.ref}:klant` };
}
...
function orderShipped(ctx) {
  ...
  return { to: client.email, subject, html, text, replyTo: company.email || company.opsEmail, idempotencyKey: `kade:${order.ref}:onderweg` };
}
...
function orderDelivered(ctx) {
  ...
  return { to: client.email, subject, html, text, replyTo: company.email || company.opsEmail, idempotencyKey: `kade:${order.ref}:factuur` };
}
...
function requestProspect(ctx) {
  ...
  return { to: request.email, subject, html, text, replyTo: company.email || company.opsEmail, idempotencyKey: `kade:aanvraag:${request.id}:klant` };
}
...
function credentials(ctx) {
  ...
  return { to: client.email, subject, html, text, replyTo: company.email || company.opsEmail, idempotencyKey: `kade:toegang:${client.id}:${Date.now()}` };
}
...
const templates = { orderTeam, orderClient, orderShipped, orderDelivered, invoiceTeamCopy, requestProspect, requestTeam, credentials };
