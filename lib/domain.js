...
const UNAVAILABLE = "NIET GELEVERD";
function isUnavailable(l) { return !l.qty || /\bNIET GELEVERD\b/i.test(String(l.comment || "")); }
function lineTotalCents(l) {
  if (!Number.isFinite(l.priceCents) || isUnavailable(l)) return 0;
...
function buildOrderLines(items, products, negotiated) {
  ...
  for (const it of items) {
    ...
    const qty = Math.round(Number(it && it.qty) * 1000) / 1000;
    const p = products.get(id);
    if (!p || p.active === false) { const e = new DomainError((p ? p.name : "Een artikel") + " is niet meer beschikbaar en werd uit uw bestelling gehaald."); e.extra = { productId: id }; throw e; }
...
// ---- Notities: klantopmerking + interne regels in één veld ----------------------------
// Het Notes-veld bevat: optioneel "[Bron] " vooraan, de opmerking van de klant, en
// daarna regels die met "[intern]" beginnen (nooit zichtbaar voor de klant).
function splitNotes(raw) {
  const text = String(raw || "");
  const m = text.match(/^\[([^\]]+)\]\s*/);
  const source = m && !/^intern$/i.test(m[1]) ? m[1] : "";
  const rest = source ? text.slice(m[0].length) : text;
  const lines = rest.split("\n");
  const customer = [], internal = [];
  lines.forEach((l) => { if (/^\[intern\]/i.test(l.trim())) internal.push(l.trim().replace(/^\[intern\]\s*/i, "")); else customer.push(l); });
  return { source, customer: customer.join("\n").trim(), internal: internal.filter(Boolean) };
}
function joinNotes({ source, customer, internal }) {
  const head = source ? "[" + source + "] " : "";
  const body = String(customer || "").trim();
  const tail = (internal || []).filter(Boolean).map((l) => "[intern] " + String(l).replace(/[\r\n]+/g, " ").trim()).join("\n");
...
module.exports = {
  ...
  serializeLine, serializeLines, parseLine, parseLines, lineTotalCents, linesTotalCents, cleanComment, cleanName, isUnavailable, UNAVAILABLE,
  ...
  vatBreakdown, splitNotes, joinNotes,
