...
async function placeOrder(p) {
  ...
  // Dezelfde bestelling twee keer (netwerkstoring)? Dan bestaande teruggeven.
  const key = String(p.key || "").replace(/[^a-zA-Z0-9-]/g, "").slice(0, 40);
  if (key) {
    const recent = await repo.listOrders({ sinceDate: dom.addDays(today, -1), clientId: p.client.id });
    const dup = recent.find((o) => dom.splitNotes(o.notes).internal.some((l) => l === "sleutel " + key));
    if (dup) return { order: dup, mail: {}, duplicate: true };
  }
  ...
  const fields = {
    ...
    [F.order.notes]: dom.joinNotes({ source: p.source && p.source !== "Klantportaal" ? p.source : "", customer: notes, internal: key ? ["sleutel " + key] : [] }),
 ...
 */
async function updateLines(order, input, { basis } = {}) {
  ...
  if (basis != null && String(basis) !== order.linesText) throw new DomainError("Deze bestelling werd intussen op een ander toestel gewijzigd. Open ze opnieuw.", 409);
  ...
  for (const raw of input) {
    ...
    const unavailable = raw && (raw.unavailable === true || /NIET GELEVERD/i.test(String(raw.comment || "")));
    if (!Number.isFinite(qty) || qty < 0 || qty > 10000 || (qty === 0 && !unavailable)) throw new DomainError(`${dom.cleanName(raw && raw.name) || "Artikel"}: ongeldig aantal.`);
    let line;
    if (raw.productId) {
      await loadCatalogue();
    ...
    if (unavailable) { line.qty = 0; line.comment = dom.UNAVAILABLE + (raw.reason ? " — " + dom.cleanComment(raw.reason) : ""); }
    lines.push(line);
  }
  ...
  before.forEach((l, k) => { const n = after.get(k); if (!n) changes.push(`${l.name}: geschrapt (was ${dom.fmtQty(l.qty)} ${dom.unitPlural(l.unit, l.qty)})`); else if (dom.isUnavailable(n) && !dom.isUnavailable(l)) changes.push(`${l.name}: niet geleverd (was ${dom.fmtQty(l.qty)} ${dom.unitPlural(l.unit, l.qty)})`); else if (n.qty !== l.qty) changes.push(`${l.name}: ${dom.fmtQty(l.qty)} → ${dom.fmtQty(n.qty)} ${dom.unitPlural(n.unit, n.qty)}`); });
...
async function markReady(order, { photo } = {}) {
  ...
  if (order.lines.every((l) => dom.isUnavailable(l)) && !photo && !arguments[1].force) throw new DomainError("Alle artikelen staan op niet geleverd. Verwijder de bestelling of verwittig de klant.", 409);
  const warnings = [];
  const img = decodeImage(photo, 4 * 1024 * 1024);
  if (img) {
...
async function markDelivered(order, { receivedBy, signature, paid }, req, { dryRun } = {}) {
  ...
  if (paid === true) fields[F.order.payment] = PAYMENT.PAID;
  ...
  const dueDate = docsLib.dueDateFor(dom.todayISO(new Date(updated.invoicedAt || Date.now())), config.paymentTerms);
  const ctx = { order: updated, client: client || { name: "", email: "" }, company: config, portalUrl: portalUrl(req), docLinks: docLinks(updated, req), vat, dueDate };
  const messages = {};
...
/** Toegang blokkeren: wachtwoord leegmaken; lopende sessies vervallen door de wachtwoordversie. */
async function blockAccess(clientId) {
  await repo.updateClient(clientId, { [F.client.password]: "" });
  return true;
}
...
async function sendCredentials(client, password, req, { contact } = {}) {
  if (!client.email) throw new DomainError("Deze klant heeft geen e-mailadres.");
  const config = await repo.getConfig();
  const m = mail.templates.credentials({ client, company: config, username: client.username, password, portalUrl: portalUrl(req), cutoff: dom.fmtCutoff(dom.parseCutoff(config.cutoff)), contact: contact || (client.notes.match(/Contactpersoon:\s*([^\n]+)/) || [])[1] || "" });
  return mail.send(m);
}
...
async function approveRequest(request, input, req) {
  ...
  const { client, password } = await saveClient({
    ...
    vat: input.vat || request.vat || "",
    username: input.username || "",
    force: input.force === true,
    notes: [request.contact ? "Contactpersoon: " + request.contact : "", request.notes ? "Bericht bij aanvraag: " + request.notes.replace(/^Btw:[^\n]*\n?/i, "") : ""].filter(Boolean).join("\n")
  });
  ...
  let sent = null;
  if (input.sendMail !== false && client.email) sent = await sendCredentials(client, password, req, { contact: request.contact });
...
async function submitRequest(input, req) {
  ...
  const vat = dom.normalizeVat(input.vat);
  const fields = {
    [F.request.company]: company, [F.request.contact]: contact, [F.request.email]: email, [F.request.phone]: phone,
    [F.request.address]: dom.cleanMultiline(input.address, 300), [F.request.notes]: dom.cleanMultiline((vat ? "Btw: " + vat + "\n" : "") + String(input.notes || ""), 600), [F.request.status]: REQUEST_STATUS.NEW
  };
...
module.exports = {
  ...
  setInternalNote, setCustomerNote, setDeliveryDate, setReceiver, addDeliveryPhoto, resendOrderMail, copyPrices, blockAccess,
