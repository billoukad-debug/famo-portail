...
const noteText = (s) => dom.splitNotes(s).customer;
const internalNotes = (s) => dom.splitNotes(s).internal;
...
.strike { text-decoration: line-through; color: #5B6B7C; }
...
@media print { body { background: #fff; } .toolbar { display: none; } .sheet { box-shadow: none; margin: 0; padding: 0; width: auto; break-after: page; } .sheet:last-child { break-after: auto; } }
...
function page(title, inner, opts = {}) {
  const back = opts.backHref ? `<a class="s" href="${esc(opts.backHref)}" id="back">Terug</a>` : `<a class="s" href="/">Naar het portaal</a>`;
  return `<!doctype html><html lang="nl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex"><title>${esc(title)}</title><style>${CSS}</style></head><body>` +
    `<div class="toolbar"><div class="t">${esc(title)}</div>${back}<button class="p" onclick="window.print()">Afdrukken</button></div>` +
    (opts.sheets ? opts.sheets.map((s) => `<div class="sheet">${s}</div>`).join("") : `<div class="sheet">${inner}</div>`) + `<script>(function(){var b=document.getElementById("back");if(b&&(window.opener||history.length>1)){b.textContent="Sluiten";b.addEventListener("click",function(e){e.preventDefault();if(window.opener){window.close();}else{history.back();}});}})();</script></body></html>`;
}
...
function linesTable(lines, { prices = true, checkboxes = false } = {}) {
  ...
  const rows = lines.map((l) => { const off = dom.isUnavailable(l); return `<tr class="${off ? "off" : ""}">${checkboxes ? '<td>' + (off ? "" : '<span class="chk"></span>') + "</td>" : ""}<td><span class="${off ? "strike" : ""}">${esc(l.name)}</span>${off ? ' <span class="stamp open" style="font-size:8pt;transform:none">NIET GELEVERD</span>' : ""}${l.comment && !off ? `<div class="c">${esc(l.comment)}</div>` : ""}</td>` +
    `<td class="num">${off ? "—" : esc(dom.fmtQty(l.qty)) + " " + esc(dom.unitPlural(l.unit, l.qty))}</td>` +
    (prices ? `<td class="num">${Number.isFinite(l.priceCents) && !off ? esc(dom.fmtEur(l.priceCents)) : "—"}</td><td class="num">${Number.isFinite(l.priceCents) && !off ? esc(dom.fmtEur(dom.lineTotalCents(l))) : "—"}</td>` : "") + "</tr>"; }).join("");
  return `<table class="lines">${head}${rows}</table>`;
}

/** Leveringsbon: zonder prijzen, met ontvangstvak. */
function deliveryNoteInner({ order, client, company }) {
  return (
...
function renderDeliveryNote({ order, client, company, backHref }) {
  return page(`Leveringsbon ${order.ref}`, deliveryNoteInner({ order, client, company }), { backHref });
}
/** Alle leveringsbonnen van een dag, één per pagina. */
function renderDeliveryNotes({ date, orders, clientsById, company, backHref }) {
  const sheets = orders.length ? orders.map((o) => deliveryNoteInner({ order: o, client: clientsById[o.clientId] || { name: "—" }, company })) : [`<p>Geen leveringen op ${esc(dom.fmtDateNl(date))}.</p>`];
  return page(`Leveringsbonnen ${dom.fmtDateShort(date)}`, "", { backHref, sheets });
}

/** Factuur: wettelijke vermeldingen, btw-uitsplitsing, betaalgegevens. */
function dueDateFor(invoiceDate, terms) { const m = /(\d+)\s*dag/i.exec(String(terms || "")); return m ? dom.addDays(invoiceDate, Number(m[1])) : invoiceDate; }
function renderInvoice({ order, client, company, backHref }) {
  ...
  const dueDate = dueDateFor(invoiceDate, company.paymentTerms);
  const inner =
    ...
    `<div class="meta">${metaBox("Factuurnummer", order.invoiceNumber || "—")}${metaBox("Factuurdatum", dom.fmtDateShort(invoiceDate))}${metaBox("Leverdatum", dom.fmtDateShort(deliveredDate))}${metaBox(order.paid ? "Betaald" : "Vervaldatum", order.paid ? "ja" : dom.fmtDateShort(dueDate))}</div>` +
    ...
    `<div class="sign"><div class="box"><div class="label">Betaling</div>` + (order.paid ? `<div><b>Betaald</b> — contant bij levering op ${esc(dom.fmtDateShort(deliveredDate))}.</div>` : `<div><b>${esc(company.paymentTerms)}</b> · vóór ${esc(dom.fmtDateShort(dueDate))}</div>` + (company.iban ? `<div>Over te schrijven op <b>${esc(company.iban)}</b>${company.bic ? " (BIC " + esc(company.bic) + ")" : ""}<br>Mededeling: <b>${esc(order.invoiceNumber || order.ref)}</b></div>` : `<div>Betaalgegevens ontvangt u van ons; neem gerust contact op.</div>`)) + `<div class="c" style="margin-top:6px;color:#5B6B7C">Bestelreferentie ${esc(order.ref)}</div></div>` +
    ...
    `<div class="foot">${esc(company.companyName)}${company.vat ? " · Btw " + esc(company.vat) : ""}${company.street ? " · " + esc(company.street) + ", " + esc(company.city) : ""}${company.email ? " · " + esc(company.email) : ""}${company.phone ? " · " + esc(company.phone) : ""}<br>${esc(company.paymentTerms || "")}</div>`;
...
function renderPicklist({ date, orders, clientsById, company, backHref }) {
  ...
  const inner =
    ...
    orders.map((o) => {
      ...
      const late = o.deliveryDate < date ? ` <span class="stamp open" style="font-size:9pt">TE LAAT · ${esc(dom.fmtDateShort(o.deliveryDate))}</span>` : "";
      return `<div class="pick"><h2><span>${esc(c.name)}${late}</span><span>${esc(o.ref)}</span></h2><div class="addr">${esc(c.address || "")}${c.phone ? " · " + esc(c.phone) : ""}</div>` +
        ...
        (noteText(o.notes) ? `<div class="note"><b>Opmerking klant:</b> ${nl2br(noteText(o.notes))}</div>` : "") + (internalNotes(o.notes).length ? `<div class="note" style="background:#EAF0F5;border-color:#D9E1EA"><b>Intern:</b> ${internalNotes(o.notes).map(esc).join("<br>")}</div>` : "") + "</div>";
    }).join("") +
...
module.exports = { renderDeliveryNote, renderDeliveryNotes, renderInvoice, renderPicklist, dueDateFor, esc };
