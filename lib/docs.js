...
function renderInvoice({ order, client, company, backHref }) {
  ...
  const inner =
    ...
    `<div class="meta">${metaBox("Factuurnummer", order.invoiceNumber || "—")}${metaBox("Factuurdatum", dom.fmtDateShort(invoiceDate))}${metaBox("Leverdatum", dom.fmtDateShort(order.deliveryDate || order.date))}${metaBox("Referentie", order.ref)}</div>` +
    ...
    (company.iban ? `<div>Over te schrijven op <b>${esc(company.iban)}</b>${company.bic ? " (BIC " + esc(company.bic) + ")" : ""}<br>Mededeling: <b>${esc(order.invoiceNumber || order.ref)}</b></div>` : `<div style="color:#B07F00">Bankgegevens nog niet ingevuld in Beheer → Bedrijf.</div>`) + `</div>` +
