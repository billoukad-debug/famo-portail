...
function publicConfig(c) {
  return {
    ...
    cutoff: c.cutoff, deliveryDays: c.deliveryDays, paymentTerms: c.paymentTerms, deliveryTerms: c.deliveryTerms, vatRate: c.vatRate
  };
}
...
function mapRequest(r) {
  ...
  return {
    ...
    address: str(c[k.address]),
    notes: str(c[k.notes]),
    vat: (str(c[k.notes]).match(/^Btw:\s*([^\n]+)/i) || [])[1] || "",
    outcome: /\[Klant aangemaakt\]/.test(str(c[k.notes])) ? "klant" : (/\[Afgehandeld\]/.test(str(c[k.notes])) ? "afgewezen" : ""),
