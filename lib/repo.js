...
function mapConfig(r) {
  ...
  return {
    ...
    vatRate: num(c[k.vatRate]) > 0 ? num(c[k.vatRate]) : DEFAULTS.vatRate,
...
async function saveConfig(id, fields) {
  if (id) { const [r] = await at.update(T.CONFIG, [{ id, fields }]); return mapConfig(r); }
  const [r] = await at.create(T.CONFIG, [fields]);
  return mapConfig(r);
}
...
async function findClientForLogin(login) {
  ...
  if (!r) return null;
  return { client: mapClient(r), storedPassword: str(f(r)[F.client.password]) };
}
...
async function negotiatedFor(clientId) {
  ...
  const map = new Map();
  all.filter((p) => p.clientId === clientId).forEach((p) => map.set(p.productId, p.priceCents));
  return map;
}
...
async function deletePriceFor(clientId, productId) {
  const all = await listPrices();
  const hits = all.filter((p) => p.clientId === clientId && p.productId === productId);
...
module.exports = {
  mapConfig, getConfig, publicConfig, saveConfig,
  mapClient, listClients, getClient, findClientForLogin, usernameTaken, createClient, updateClient, deleteClient,
