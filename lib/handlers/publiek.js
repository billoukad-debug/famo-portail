...
module.exports = (router) => {
  ...
  router.get("publiek/config", async ({ res }) => {
    ...
    json(res, 200, { configured: true, company: repo.publicConfig(c), cutoff: dom.fmtCutoff(dom.parseCutoff(c.cutoff)), deliveryDays: Array.from(dom.parseDeliveryDays(c.deliveryDays)).map((d) => dom.DAY_LONG[d]) });
  });
  ...
  router.post("publiek/aanvraag", async ({ req, res, body }) => {
    ...
    const r = await service.submitRequest({ company: body.bedrijfsnaam, contact: body.contactpersoon, email: body.email, phone: body.telefoon, address: body.adres, notes: body.bericht }, req);
