...
async function overview(req) {
  ...
  const warnings = [];
  if (!config.id) warnings.push({ key: "config", text: "Vul de bedrijfsgegevens in: ze staan op elke leveringsbon en factuur." });
  if (!config.iban) warnings.push({ key: "iban", text: "Het IBAN ontbreekt: zonder rekeningnummer kan de klant niet betalen." });
  if (!mail.enabled()) warnings.push({ key: "mail", text: "E-mail is niet ingeschakeld (RESEND_API_KEY ontbreekt). Er vertrekt geen enkele e-mail." });
  else if (!config.opsEmail) warnings.push({ key: "ops", text: "Geen interne postbus ingesteld: bestelmeldingen gaan naar het algemene e-mailadres" + (config.email ? " " + config.email : " — en dat is leeg") + "." });
  if (!products.some((p) => p.active)) warnings.push({ key: "catalogue", text: "De catalogus heeft geen actieve artikelen: klanten kunnen niets bestellen." });
  const noEmail = clients.filter((c) => !c.email).length;
  if (noEmail) warnings.push({ key: "clientmail", text: `${noEmail} klant${noEmail > 1 ? "en" : ""} zonder e-mailadres: geen bevestigingen of facturen per e-mail.` });
  const noPassword = clients.filter((c) => !c.hasPassword).length;
  if (noPassword) warnings.push({ key: "clientpw", text: `${noPassword} klant${noPassword > 1 ? "en" : ""} zonder wachtwoord: geef een wachtwoord om te kunnen bestellen.` });
...
module.exports = (router) => {
  ...
  router.post("beheer/klanten", async ({ req, res, body }) => {
    requireAdmin(req);
    const r = await service.saveClient({ id: body.id ? String(body.id) : "", name: body.naam, email: body.email, phone: body.telefoon, address: body.adres, vat: body.btw, number: body.klantnummer, username: body.gebruikersnaam, notes: body.notities, usual: body.vasteArtikelen });
    json(res, 200, { ok: true, client: r.client, password: r.password, overview: await overview(req) });
  });
  ...
  router.post("beheer/klanten/:id/verwijderen", async ({ req, res, params }) => {
    ...
    await repo.deletePriceFor(client.id, "*").catch(() => 0);
    const prices = (await repo.listPrices()).filter((p) => p.clientId === client.id);
    for (const p of prices) await repo.deletePriceFor(client.id, p.productId);
  ...
  router.post("beheer/aanvragen/:id/goedkeuren", async ({ req, res, params, body }) => {
    ...
    const r = await service.approveRequest(request, { name: body.naam, email: body.email, phone: body.telefoon, address: body.adres, vat: body.btw, username: body.gebruikersnaam, sendMail: body.stuurMail !== false }, req);
  ...
  router.post("beheer/aanvragen/:id/afhandelen", async ({ req, res, params, body }) => {
    ...
    const request = await repo.getRequest(params.id).catch(() => null);
    if (!request) throw new HttpError(404, "Aanvraag niet gevonden.");
    await service.closeRequest(request, dom.clean(body.notitie, 300));
    json(res, 200, { ok: true, overview: await overview(req) });
  });
