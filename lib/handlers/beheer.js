...
module.exports = (router) => {
  ...
  router.post("beheer/klanten/:id/wachtwoord", async ({ req, res, params, body }) => {
    ...
    if (body.blokkeren === true) { await service.blockAccess(client.id); return json(res, 200, { ok: true, blocked: true }); }
  ...
  router.post("beheer/klanten/:id/verwijderen", async ({ req, res, params }) => {
    ...
    if (orders.length) throw new HttpError(409, `Deze klant heeft ${orders.length} bestelling${orders.length > 1 ? "en" : ""} en kan niet worden verwijderd. Gebruik "Toegang blokkeren".`);
  ...
  router.post("beheer/aanvragen/:id/goedkeuren", async ({ req, res, params, body }) => {
    ...
    let r;
    try { r = await service.approveRequest(request, { name: body.naam, email: body.email, phone: body.telefoon, address: body.adres, vat: body.btw, username: body.gebruikersnaam, sendMail: body.stuurMail !== false, force: body.force === true }, req); }
    catch (e) { if (e && e.status === 409) { const all = await repo.listClients(); const norm = (s) => String(s || "").toLowerCase().replace(/[^a-z0-9]/g, ""); const dup = all.find((c) => (body.email && c.email === String(body.email).toLowerCase()) || norm(c.name) === norm(body.naam || request.company)); e.extra = { duplicateId: dup ? dup.id : "" }; } throw e; }
