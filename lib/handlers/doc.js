...
module.exports = (router) => {
  ...
  // Alle leveringsbonnen van een dag in één afdruk (één per pagina).
  router.get("doc/leveringsbonnen", async ({ req, res, query }) => {
    ...
    const list = orders.filter((o) => o.deliveryDate === date || (o.deliveryDate < date && o.statusKey !== "geleverd")).sort((a, b) => ((clients.find((c) => c.id === a.clientId) || {}).name || "").localeCompare((clients.find((c) => c.id === b.clientId) || {}).name || "", "nl"));
    const clientsById = {};
    clients.forEach((c) => { clientsById[c.id] = c; });
    html(res, 200, docs.renderDeliveryNotes({ date, orders: list, clientsById, company, backHref: "/team" }));
  });
  ...
  router.get("doc/picklijst", async ({ req, res, query }) => {
    ...
    const list = orders.filter((o) => o.deliveryDate <= date && o.statusKey !== "geleverd").sort((a, b) => a.deliveryDate.localeCompare(b.deliveryDate) || a.ref.localeCompare(b.ref));
    const clientsById = {};
    clients.forEach((c) => { clientsById[c.id] = c; });
