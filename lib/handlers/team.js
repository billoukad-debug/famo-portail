...
function orderView(o, clientsById, req) {
  ...
  const source = (o.notes.match(/^\[([^\]]+)\]/) || [])[1] || "Klantportaal";
  return {
    ...
    notes: o.notes.replace(/^\[[^\]]+\]\s*/, ""), source,
...
module.exports = (router) => {
  ...
  router.get("team/bestellingen/:id", async ({ req, res, params }) => {
    ...
    json(res, 200, { order: orderView(o, c ? { [c.id]: c } : {}, req) });
  });
  ...
  router.post("team/bestellingen/:id/notitie", withOrder(async ({ req, res, body }, o) => {
    const source = (o.notes.match(/^\[([^\]]+)\]\s*/) || [])[0] || "";
    await reply(res, await service.setNotes(o, source + dom.cleanMultiline(body.notities, 900)), null, req);
  }));
