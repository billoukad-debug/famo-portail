...
function orderView(o, clientsById, req) {
  ...
  const vat = dom.vatBreakdown(o.totalCents, vatRateCache);
  ...
  return {
    ...
    paid: o.paid, paymentLabel: dom.paymentLabel(o.payment), totalCents: o.totalCents, vat,
    lines: o.lines.map((l) => ({ name: l.name, qty: l.qty, unit: l.unit, unitLabel: dom.unitLabel(l.unit), unitPlural: dom.unitPlural(l.unit, l.qty), priceCents: l.priceCents, comment: l.comment, decimals: dom.allowsDecimals(l.unit), unparsed: !!l.unparsed, unavailable: dom.isUnavailable(l) && !l.unparsed })),
    notes: notes.customer, internalNotes: notes.internal, source,
    invoiceNumber: o.invoiceNumber, prepValidated: o.prepValidated, preparedAt: o.preparedAt, deliveredAt: o.deliveredAt, invoicedAt: o.invoicedAt, linesText: o.linesText,
...
module.exports = (router) => {
  router.post("team/login", async ({ req, res, body }) => {
    ...
    if (body.rol === "admin" && role !== "admin") throw new HttpError(403, "Dit is de teamcode. Beheer vraagt de beheerderscode.");
    auth.rateReset(key);
  ...
  router.post("team/bestellingen/:id/lijnen", withOrder(async ({ req, res, body }, o) => {
    const updated = await service.updateLines(o, Array.isArray(body.lijnen) ? body.lijnen : [], { basis: body.basis });
    await reply(res, updated, null, req);
  }));
  router.post("team/bestellingen/:id/klaar", withOrder(async ({ req, res, body }, o) => {
    const r = await service.markReady(o, { photo: body.foto, force: body.force === true });
  ...
  router.post("team/bestellingen/:id/geleverd", withOrder(async ({ req, res, body }, o) => {
    const r = await service.markDelivered(o, { receivedBy: body.ontvanger, signature: body.handtekening, paid: body.betaald === true }, req);
    await reply(res, r.order, { mail: r.mail, warnings: r.warnings, invoiceNumber: r.invoiceNumber }, req);
  }));
