...
function orderView(o, req, vatRate) {
  ...
  return {
    ...
    totalCents: o.totalCents, lines: o.lines.map((l) => ({ name: l.name, qty: l.qty, unit: l.unit, unitLabel: dom.unitPlural(l.unit, l.qty), priceCents: l.priceCents, comment: dom.isUnavailable(l) ? "" : l.comment, unavailable: dom.isUnavailable(l) })),
...
module.exports = (router) => {
  router.post("klant/login", async ({ req, res, body }) => {
    ...
    auth.setClientSession(res, hit.client.id, hit.passwordVersion, { short: body.blijf === false });
  ...
  router.post("klant/bestellen", async ({ req, res, body }) => {
    ...
    const r = await service.placeOrder({ client, items, deliveryDate: body.leverdatum, notes: body.opmerking, source: "Klantportaal", req, key: body.sleutel });
    json(res, 200, { ok: true, order: orderView(r.order, req), mail: r.mail, duplicate: !!r.duplicate });
  });
