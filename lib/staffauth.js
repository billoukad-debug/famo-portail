...
// Jeton : exp.role[.naam].sig — le nom (compte Medewerkers) est optionnel et encodé
// base64url ; un jeton à trois segments (code partagé) reste valable tel quel.
function sign(expMs, role, name) {
  const n = name ? Buffer.from(String(name)).toString("base64url") : "";
  const p = String(expMs) + "." + role + (n ? "." + n : "");
...
function verify(tok) {
  ...
  const [expStr, role] = parts;
  const n = parts.length === 4 ? parts[2] : "";
  if (role !== "staff" && role !== "admin") return null;
  let good;
  try {
    good = crypto.createHmac("sha256", secret()).update(expStr + "." + role + (n ? "." + n : "")).digest("base64url");
  } catch (e) {
    return null;
  }
  ...
  return { role, exp: Number(expStr), name };
}
